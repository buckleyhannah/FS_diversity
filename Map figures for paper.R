## FS diversity paper map figures code
...
other.spp.list <- as.character(unique(dat$sp[!(dat$sp %in% spp.list)]))
    
ppp.ls.other <- vector("list", length(other.spp.list))  # empty list to hold all OTHER species' point patterns
...
gdat5 %>%
  ...
  mutate(Statistic = (measurement - mean(measurement))/sd(measurement)) %>%
  ...
  ggplot(aes(x = x, y = y, fill = Statistic)) +
...
ggsave("All_datasets_FS_stats_5m.png", width = 8, height = 8, dpi = 600)
...
gdat5 %>%
  ...
  ggplot(aes(x = x, y = y, fill = Diversity)) +
    ...
    xlab("X (m)") + ylab("Y (m)") +
    theme_bw(base_size = 16)
ggsave("All_datasets_community_stats_5m.png", width = 12, height = 8, dpi = 600)
