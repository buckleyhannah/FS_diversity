...
ppp.other <- ppp(dat$gx[dat$sp %in% other.spp.list], dat$gy[dat$sp %in% other.spp.list], xrange = c(xmin, xmax), yrange = c(ymin, ymax), marks = dat$sp[dat$sp %in% other.spp.list])

rnd.stats <- rep(list(as.list(numeric(n.rand))),5)

for(rand in 1:n.rand) {
  tor.other <- rshift(ppp.other)
  tor.ppp.ls.other <- split(tor.other)
  rnd.stats[[1]][[rand]] <- geoR.to.matrix.fn(ppp.diversity.to.geoR.fn(tor.ppp.ls.other, spp.list = other.spp.list, xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax, index = "total.abundance", quad.size = quad.size))
  rnd.stats[[2]][[rand]] <- geoR.to.matrix.fn(ppp.diversity.to.geoR.fn(tor.ppp.ls.other, spp.list = other.spp.list, xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax, index = "richness", quad.size = quad.size))
  rnd.stats[[3]][[rand]] <- geoR.to.matrix.fn(ppp.diversity.to.geoR.fn(tor.ppp.ls.other, spp.list = other.spp.list, xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax, index = "shannon", quad.size = quad.size))
  rnd.stats[[4]][[rand]] <- geoR.to.matrix.fn(ppp.diversity.to.geoR.fn(tor.ppp.ls.other, spp.list = other.spp.list, xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax, index = "simpson", quad.size = quad.size))
  rnd.stats[[5]][[rand]] <- geoR.to.matrix.fn(ppp.diversity.to.geoR.fn(tor.ppp.ls.other, spp.list = other.spp.list, xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax, index = "bray-curtis.dissimilarity", quad.size = quad.size))
  }
...
save.image("Results_ToroidalShift_19-01-06.RData")
#load("Results_ToroidalShift_19-01-06.RData")
...
ggsave(filename = paste("tor_obs", dataset.names[d],unique(sdat$species)[s],quad.size,".jpg", sep="_"), width = 12, height = 6)
...
gdat %>% 
  ...
  xlab("Spatial lag in X (m)") + ylab("Spatial lag in Y (m)") +
  theme_bw(base_size = 16)
ggsave(filename = paste("tor_mean_null", dataset.names[d],unique(sdat$species)[s],quad.size,".jpg", sep="_"), width = 12, height = 6)
...
gdat %>% 
  ...
  xlab("Spatial lag in X (m)") + ylab("Spatial lag in Y (m)") +
  theme_bw(base_size = 16)
ggsave(filename = paste("tor_sig", dataset.names[d],unique(sdat$species)[s],quad.size,".jpg", sep="_"), width = 14, height = 8)
...
ggsave(filename = paste("tor", site, "Total_basal_area", spp.layer, quad.size, "codisp", ".jpg", sep="_"), width = 20, height = 10)
...
ggsave(filename = paste("tor", site, "Total_basal_area", spp.layer, quad.size, "sig", ".jpg", sep="_"), width = 20, height = 10)
...
g.output %>%
  ...
  theme_bw(base_size = 16) +
  theme(strip.text.x = element_text(face = "italic"))
ggsave(filename = paste("tor", TT, "understory", "Total_basal_area", quad.size, "codisp", ".jpg", sep="_"), width = 20, height = 10)
...
g.output %>%
  ...
  theme_bw(base_size = 16) +
  theme(strip.text.x = element_text(face = "italic"), legend.position = "none")
ggsave(filename = paste("tor", TT, "understory", "Total_basal_area", quad.size, "sig", ".jpg", sep="_"), width = 20, height = 10)
...
write.csv(tab1, "tor_Codisp_results_table.csv", row.names = F)
