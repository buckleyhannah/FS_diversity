...
# Note that if there are empty quadrats with zero species, pielou evenness and simpson evenness will be zero
# Note that quadrats with one species only are given pielou evenness of 1
# This function requires the library 'vegan'

ppp.diversity.to.geoR.fn <- function(ppp.ls = ppp.ls, spp.list = spp.list, xmin, xmax, ymin, ymax, quad.size, index = c("total.abundance", "richness", "shannon", "simpson", "pielou.evenness", "simpson.evenness") ){
  ...
  names(spp.df) <- spp.list  # put species names on dataframe
  ...
  if(index == "simpson") { 
    ...
    out.vec[out.vec == Inf] <- 0 } # replace quadrats with zero species with 0 simpson
  if(index == "pielou.evenness") { 
    out.vec <- diversity(spp.df, index = "shannon")/log(specnumber(spp.df))
    out.vec[is.na(out.vec) == TRUE] <- 1 } # replace quadrats with one species with 1 evenness 
  if(index == "simpson.evenness") { 
    out.vec <- diversity(spp.df, index = "invsimpson") * (1 / specnumber(spp.df))
    out.vec[out.vec == Inf] <- 0  } # replace quadrats with zero species with 0 evenness 
